...
impl TripleRatchetParticipant {
    ...
    pub fn ratchet_decrypt(&mut self, envelope: &P2PChannelEnvelope) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
      ...
      let (header, _) = self.decrypt_header(&envelope.message_header, &self.current_header_key)?;

      let (sender_key, receiving_ephemeral_key, previous_receiving_chain_length, current_receiving_chain_length) = 
  ...
  fn try_skipped_message_keys(&self, envelope: &P2PChannelEnvelope) -> Result<Option<Vec<u8>>, Box<dyn std::error::Error>> {
      for (receiving_header_key, skipped_keys) in &self.skipped_keys_map {
          if let Ok((header, _)) = self.decrypt_header(&envelope.message_header, receiving_header_key) {
              let (peer_key, _, _, current) = self.decode_header(&header)?;
  ...
  fn decrypt_header(&self, ciphertext: &MessageCiphertext, receiving_header_key: &[u8]) -> Result<(Vec<u8>, bool), Box<dyn std::error::Error>> {
  ...
  fn decode_header(&self, header: &[u8]) -> Result<(EdwardsPoint, Scalar, u32, u32), TripleRatchetError> {
      if header.len() < 9 {
      ...
      let current_receiving_chain_length = u32::from_be_bytes(header[117..].try_into().unwrap());

      Ok((sender_key, receiving_ephemeral_key, previous_receiving_chain_length, current_receiving_chain_length))
  }
